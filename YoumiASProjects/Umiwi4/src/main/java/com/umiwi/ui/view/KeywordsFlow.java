...
import com.umiwi.ui.util.ColorUtil;
...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
    ...
    /** 存储显示的关键字。 */  
    private Vector<SearchCloudBean> searchCloudBeans;  
    ...
    private void init() {  
        ...
        random = new Random();  
        searchCloudBeans = new Vector<SearchCloudBean>(MAX);  
    ...
    public boolean feedKeyword(SearchCloudBean keyword) {  
        boolean result = false;  
        if (searchCloudBeans.size() < MAX) {  
            result = searchCloudBeans.add(keyword);  
        }  
        return result;  
    }  
    ...
    private boolean show() {
        if (width > 0 && height > 0 && searchCloudBeans != null && searchCloudBeans.size() > 0 && enableShow) {  
            enableShow = false;  
            lastStartAnimationTime = System.currentTimeMillis();  
            int xCenter = width >> 1, yCenter = height >> 1;  
            int size = searchCloudBeans.size();  
            ...
            for (int i = 0; i < size; i++) {  
            	SearchCloudBean bean = searchCloudBeans.get(i);  
                // 随机颜色   
                ...
                //txt.setBackgroundResource(backType);
                txt.setTag(R.id.key_search_cloud_key, bean);
                txt.setOnClickListener(itemClickListener);  
                txt.setText(bean.getTitle());
                
                int color = ColorUtil.transferFromString(bean.getColor());
                if(color == -1) {
                    if (AndroidSDK.isMarshmallow()) {
                        color = getResources().getColor(R.color.umiwi_gray_3,null);
                    } else {
                        color = getResources().getColor(R.color.umiwi_gray_3);
                    }
                }
                ...
                int strWidth = (int) Math.ceil(paint.measureText(bean.getTitle()));  
    ...
    public Vector<SearchCloudBean> getKeywords() {  
        return searchCloudBeans;  
    }  
  
    public void rubKeywords() {  
        searchCloudBeans.clear();  
    }  
